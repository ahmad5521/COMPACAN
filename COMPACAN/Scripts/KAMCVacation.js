                ...
                $.each(resultGlobal, function (key, item) {
                    ...
                    if (item.itsStateID == 12)
                        html += '<td style="vertical-align: middle;"><div><span class="label label-danger'
                            ...
                            + item.itsState + ' - ' + item.taskID + '</span></div></td>';
                    else
                    ...
                    html += '<button data-toggle="dropdown" class="btn btn-primary btn-sm dropdown-toggle"> خيارات<span class="caret"></span></button>';
...
function getbyID(vacationID) {
    ...
    $.ajax({
        url: "/KAMCVAcations/getVacationDetails/" + vacationID,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (result) {
...
function trackingbyID(vacationID) {
    $.ajax({
        ...
        success: function (result) {
            ...
            var div = '<div id="trackingTable">'; div += '<h3>الحالة:</h3>';
...
function downloadFile(id) {
    //alert(id);
    window.open('/KAMCVAcations/DownLoadVacationAttachment/' + id, 'attachment');
}
...
function PrintElem() {
    ...
    var style = '<link href="https://fonts.googleapis.com/css?family=Open+Sans:400,300,600,700" rel="stylesheet" type="text/css">'+
    ...
    mywindow.document.write('<html><head>' + style + '<title>' + document.title + '</title>');
