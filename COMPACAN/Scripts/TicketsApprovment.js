...
//function for vacation return
function vacationReturnForEdit() {
    $("#confirmModelReturnForEdit").css("margin-top", "7%");
    $('#confirmModelReturnForEdit').modal('show');
    //nextStep(20, $('#acceptNote').val());
    $('#returnForEditNote').val('');
}
...
function vacationDenay() {
    $("#confirmModelDelete").css("margin-top", "7%");
    $('#confirmModelDelete').modal('show');
    //nextStep(51, $('#denayNote').val());
    $('#denayNote').val('');
}


$('#denayConfirm').click(function () {
    if(list.filter(o => o.treansactionID_FK === currentVacationId)[0].taskID === 261)
        nextStep(51, $('#denayNote').val());
    if(list.filter(o => o.treansactionID_FK === currentVacationId)[0].taskID === 276)
        nextStep(96, $('#denayNote').val());
    if(list.filter(o => o.treansactionID_FK === currentVacationId)[0].taskID === 277)
        nextStep(97, $('#denayNote').val());
});

$('#AcceptConferm').click(function () {
    //if (list.filter(o => o.treansactionID_FK === currentVacationId)[0].taskID === 261)
    //    nextStep(91, $('#acceptNote').val());
    //if (list.filter(o => o.treansactionID_FK === currentVacationId)[0].taskID === 276)
    //    nextStep(91, $('#acceptNote').val());
    //if (list.filter(o => o.treansactionID_FK === currentVacationId)[0].taskID === 277)
        nextStep(91, $('#acceptNote').val());
...
$('#returnForEditConferm').click(function () {
    if (list.filter(o => o.treansactionID_FK === currentVacationId)[0].taskID === 261)
        nextStep(92, $('#returnForEditNote').val());
    if (list.filter(o => o.treansactionID_FK === currentVacationId)[0].taskID === 276)
        nextStep(98, $('#returnForEditNote').val());
    if (list.filter(o => o.treansactionID_FK === currentVacationId)[0].taskID === 277)
        nextStep(99, $('#returnForEditNote').val());
...
function getbyID(vacationID) {
    ...
    $.ajax({
        url: "/TicketsApprovment/getVacationDetails/" + vacationID,
        typr: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (result) {
            ...
            $.ajax({
                url: "/TicketsApprovment/PassportList/" + vacationID,
                typr: "GET",
                contentType: "application/json;charset=UTF-8",
                dataType: "json",
                success: function (result) {
...
function downloadFile(id) {
    //alert(id);
    window.open('/TicketsApprovment/DownLoadVacationAttachment/' + id, 'attachment');
}

function downloadPassprot(id) {
    //alert(id);
    window.open('/TicketsApprovment/DownLoadPassport/' + id, 'attachment');
}
